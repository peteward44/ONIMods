	...
	public void RefreshRows()
	{
		...
		if (allowRefresh)
		{
			foreach (KeyValuePair<Tag, GameObject> categoryRow in categoryRows)
			{
				...
				{
					((TMP_Text)component.GetReference<LocText>("AvailableLabel")).SetText((string)UI.ALLRESOURCESSCREEN.FIRST_FRAME_NO_DATA);
					((TMP_Text)component.GetReference<LocText>("TotalLabel")).SetText((string)UI.ALLRESOURCESSCREEN.FIRST_FRAME_NO_DATA);
					((TMP_Text)component.GetReference<LocText>("ReservedLabel")).SetText((string)UI.ALLRESOURCESSCREEN.FIRST_FRAME_NO_DATA);
					continue;
				}
				switch (units[categoryRow.Key])
				{
				case GameUtil.MeasureUnit.mass:
					((TMP_Text)component.GetReference<LocText>("AvailableLabel")).SetText(GameUtil.GetFormattedMass(amount));
					((TMP_Text)component.GetReference<LocText>("TotalLabel")).SetText(GameUtil.GetFormattedMass(totalAmount));
					((TMP_Text)component.GetReference<LocText>("ReservedLabel")).SetText(GameUtil.GetFormattedMass(totalAmount - amount));
					break;
				case GameUtil.MeasureUnit.kcal:
				{
					float calories = RationTracker.Get().CountRations(null, ClusterManager.Instance.activeWorld.worldInventory);
					((TMP_Text)component.GetReference<LocText>("AvailableLabel")).SetText(GameUtil.GetFormattedCalories(calories));
					((TMP_Text)component.GetReference<LocText>("TotalLabel")).SetText(GameUtil.GetFormattedCalories(totalAmount));
					((TMP_Text)component.GetReference<LocText>("ReservedLabel")).SetText(GameUtil.GetFormattedCalories(totalAmount - amount));
					break;
				}
				case GameUtil.MeasureUnit.quantity:
					((TMP_Text)component.GetReference<LocText>("AvailableLabel")).SetText(GameUtil.GetFormattedUnits(amount));
					((TMP_Text)component.GetReference<LocText>("TotalLabel")).SetText(GameUtil.GetFormattedUnits(totalAmount));
					((TMP_Text)component.GetReference<LocText>("ReservedLabel")).SetText(GameUtil.GetFormattedUnits(totalAmount - amount));
					break;
				}
			}
			foreach (KeyValuePair<Tag, GameObject> resourceRow in resourceRows)
			{
				...
				if (!worldInventory.HasValidCount)
				{
					((TMP_Text)component2.GetReference<LocText>("AvailableLabel")).SetText((string)UI.ALLRESOURCESSCREEN.FIRST_FRAME_NO_DATA);
					((TMP_Text)component2.GetReference<LocText>("TotalLabel")).SetText((string)UI.ALLRESOURCESSCREEN.FIRST_FRAME_NO_DATA);
					((TMP_Text)component2.GetReference<LocText>("ReservedLabel")).SetText((string)UI.ALLRESOURCESSCREEN.FIRST_FRAME_NO_DATA);
				}
				else
				{
					...
					{
					case GameUtil.MeasureUnit.mass:
						((TMP_Text)component2.GetReference<LocText>("AvailableLabel")).SetText(GameUtil.GetFormattedMass(amount2));
						((TMP_Text)component2.GetReference<LocText>("TotalLabel")).SetText(GameUtil.GetFormattedMass(totalAmount2));
						((TMP_Text)component2.GetReference<LocText>("ReservedLabel")).SetText(GameUtil.GetFormattedMass(totalAmount2 - amount2));
						break;
					case GameUtil.MeasureUnit.kcal:
					{
						...
						((TMP_Text)component2.GetReference<LocText>("AvailableLabel")).SetText(GameUtil.GetFormattedCalories(calories2));
						((TMP_Text)component2.GetReference<LocText>("TotalLabel")).SetText(GameUtil.GetFormattedCalories(totalAmount2));
						((TMP_Text)component2.GetReference<LocText>("ReservedLabel")).SetText(GameUtil.GetFormattedCalories(totalAmount2 - amount2));
						break;
					}
					case GameUtil.MeasureUnit.quantity:
						((TMP_Text)component2.GetReference<LocText>("AvailableLabel")).SetText(GameUtil.GetFormattedUnits(amount2));
						((TMP_Text)component2.GetReference<LocText>("TotalLabel")).SetText(GameUtil.GetFormattedUnits(totalAmount2));
						((TMP_Text)component2.GetReference<LocText>("ReservedLabel")).SetText(GameUtil.GetFormattedUnits(totalAmount2 - amount2));
						break;
					}
				}
